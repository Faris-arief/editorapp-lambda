...
class AuthService {
    constructor() {
        this.baseUrl = process.env.API_BASE_URL || 'https://editorapp-be.fly.dev/api';
        this.credentials = {
    ...
    async signIn() {
        try {
            ...
            if (response.status !== 200) {
                throw new Error(`Sign-in failed with status ${response.status}: ${response.statusText}`);
            }
            ...
            if (!responseData.success) {
                throw new Error(`Sign-in failed: ${responseData.message || 'Unknown error'}`);
            }
